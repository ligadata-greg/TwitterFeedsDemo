...
public class OutputJsonDAO extends GenericDAO {
		...
		try {
			...
			statement
					.execute("CREATE TABLE "
							...
							+ ".`outputdata` (`id` int(100) NOT NULL AUTO_INCREMENT,`userid` varchar(100) NOT NULL,`json` text NOT NULL,`createdon` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,  PRIMARY KEY (`id`),  KEY `id` (`id`));");
	...
	public void truncateTable() {
		...
		try {
			connection = getConnection();
			statement = connection.createStatement();
			statement.execute("TRUNCATE TABLE " + GenericDAO.getSCHEMA_NAME()
					+ ".`outputdata`;");
	...
	public GenericObjs selectLastRecord(Connection con) {
		...
		try {
			...
			preparedStatement = connection.prepareStatement("SELECT * FROM "
					+ GenericDAO.getSCHEMA_NAME()
					+ ".outputdata order by createdon desc limit 1;");
	...
	public GenericObjs selectById(int id) {
		...
		try {
			connection = getConnection();
			preparedStatement = connection.prepareStatement("SELECT * FROM "
					+ GenericDAO.getSCHEMA_NAME() + ".outputdata WHERE id = ?");
			preparedStatement.setInt(1, id);
			resultSet = preparedStatement.executeQuery();
